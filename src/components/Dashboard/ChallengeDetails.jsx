...
const ChallengeDetails = ({question,setQuestion}) => {
  ...
  const handleUpdate = async (event) => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}questions/${question.id}/`, {
        method: 'PUT',
        headers : {
  ...
  return (
    <div className={styles.challengeDetailsContainer}>
      ...
      <span className={styles.buttons}>
        <button type="submit" className={styles.submitButton} onClick={(event) => handleUpdate(event)}>
